...
try:
    ...
    from . import query_shapefile_at_point, utils
...
def get_clim_div(lat, lon):
    ...
    clim_div_shapefile = utils.find_file_or_dir(
        ROOT_FOLDER, "GIS.OFFICIAL_CLIM_DIVISIONS.shp", 1
    )
    feature_attribute_to_query = "CLIMDIV"
    clim_div = query_shapefile_at_point.check(
        lat=lat,
        lon=lon,
    ...
    try:
        ...
        if not monthly_values:
            ...
            if pdsi_file_year < int(year):
                log.Wrap(
                    "      NOAA server has yet to publish the most up-to-date file.  PDSI Unavailable."
                )
            else:
...
if __name__ == "__main__":
    palmer_value, palmer_class, palmer_color, pdsidv_file = get_pdsidv(
        lat=30, lon=-90, year="2025", month="1", pdsidv_file=None
    )
