...
def test_usgs_epqs_servers():
    ...
    try:
        ...
        log.Wrap("")
        raise Exception(
            "The APT is unable to run if USGS Elevation Point Query Services are offline."
        )
...
def time2String(total_seconds):
    if total_seconds < 61:
        ...
        output_string = "{} hours, {} minutes, and {} seconds".format(
...
class AnteProcess(object):
    ...
    def setInputs(
        self, inputList, watershed_analysis, all_sampling_coordinates, gridded
    ):
        # Set Inputs
        ...
        if self.oldLatLong is None:
            ...
            self.obs_elevation = round(
                getElev.main(
                    self.site_lat, self.site_long, epqs_variant=self.epqs_variant
                ),
                3,
            )
        else:
                ...
                if not self.watershed_analysis:
                    ...
                    self.obs_elevation = round(
                        getElev.main(
                            self.site_lat,
                            self.site_long,
                            epqs_variant=self.epqs_variant,
                        ),
                        3,
                    )
                    # self.recentStations = []
                    ...
                    if (
                        self.old_all_sampling_coordinates is None
                    ):  # First point of a watershed analysis
                        self.old_all_sampling_coordinates = (
                        ...
                        if (
                            ...
                            self.log.Wrap(
                                "New Watershed Analysis - starting new recent stations list."
                            )
                            # self.recentStations = []
                            self.all_sampling_coordinate_elevations = None
                            self.old_all_sampling_coordinates = (
        ...
        if self.save_folder is not None:
            ...
            if not folder_exists:
                self.log.Wrap(
                    "Creating output directory ({})...".format(self.folderPath)
                )
            ...
            if not folder_exists:
                self.log.Wrap(
                    "Creating stationData output directory ({})...".format(
                        self.stationFolderPath
                    )
                )
        ...
        if self.ghcn_station_list is None:
            ...
            if self.data_type == "PRCP" and self.gridded is False:
                if stations_pickle_exists:
                    ...
                    stale = file_older_than(
                        file_path=pickle_path, time_unit="days", time_value=90
                    )
                    if stale:
                        remove_pickle = True
                    else:
                    ...
                    if remove_pickle is True:
                        ...
                        self.log.Wrap(
                            'Unserializing previously cached NCDC GHCN daily weather stations..."'
                        )
                        try:
                            with open(pickle_path, "rb") as handle:
            ...
            if self.ghcn_station_list is None and self.gridded is False:
                ...
                self.ghcn_station_list = ulmo.ncdc.ghcn_daily.get_stations(
                    elements=self.data_type, update=True, as_dataframe=True
                )
                self.log.Wrap("")
                # Pickling for later
                if self.data_type == "PRCP":
    ...
    def start_multiprocessing(self):
        ...
        self.log.Wrap(
            "Preparing to use sub-processes to accelerate data acquisition..."
        )
        ...
        minions = [
            process_manager.Minion(tasks_queue, results_queue)
    ...
    def find_and_enqueue_stations(self, tasks_queue):
        ...
        self.log.Wrap(
            "Searching for weather stations within "
            + str(self.searchDistance)
            + " miles..."
        )
        ...
        for index, row in self.ghcn_station_list.iterrows():
            ...
            if ballpark:
                ...
                if self.watershed_analysis:
                    for sampling_coordinates in self.all_sampling_coordinates:
                        sampling_distance = great_circle(
                            sampling_coordinates, location_tuple
                        ).miles
                ...
                if include_station:
                    ...
                    if already is False:
                        station_number_for_print += 1
                        self.log.Wrap(
                            "Station {} - {}".format(station_number_for_print, name)
                        )
                        constructor_class = station_manager.Constructor(
                            self.data_type,
                            station_index,
                            name,
                            location,
    ...
    def finish_multiprocessing(
        self, tasks_queue, results_queue, minions, enqueue_count
    ):
            ...
            try:
                ...
                if count_copy < enqueue_count:
                    try:
                        ...
                        if count_copy < 2:
                            msg = "{} station remaining.  Approximately {} remaining.".format(
                                count_copy, remaining_string
                            )
                        else:
                            msg = (
                                "{} stations left.  Approximately {} remaining.".format(
                                    count_copy, remaining_string
                                )
                            )
            ...
            try:
                ...
                if self.watershed_analysis is True:
                    if self.all_sampling_coordinate_elevations is None:
                        self.all_sampling_coordinate_elevations = getElev.batch(
                            self.all_sampling_coordinates,
                            epqs_variant=self.epqs_variant,
                        )
            except Exception:
    ...
    def getBest(self, need_primary):
        ...
        if len(self.stations) < 1:
            ...
            if need_primary is True:
                ...
                tolerable_difference_per_thousand = 0.75
                for station in self.stations:
                ...
                if not medium_record_primary is None:
                    if not minimum_record_primary is None:
                        record_test = medium_record_primary.weightedDiff < (
                            minimum_record_primary.weightedDiff
                            + (2 * tolerable_difference_per_thousand)
                        )
                        if record_test:
                ...
                if not huge_record_primary is None:
                    if not minimum_record_primary is None:
                        record_test = huge_record_primary.weightedDiff < (
                            minimum_record_primary.weightedDiff
                            + (4 * tolerable_difference_per_thousand)
                        )
                        if record_test:
                            if not medium_record_primary is None:
                                if huge_record_primary.weightedDiff < (
                                    medium_record_primary.weightedDiff
                                    + (2 * tolerable_difference_per_thousand)
                                ):
        ...
        for station in self.stations:
            ...
            if station.name != primary_station.name:
                distance = great_circle(
                    primary_station.location, station.location
                ).miles
    ...
    def createFinalDF(self):
        ...
        if self.gridded is True:
            station_table_column_labels = [["", "Station Count Summary"]]
        ...
        if self.gridded is False:
            ...
            self.log.Wrap("")
            self.log.Wrap(
                "Creating an empty dataframe from {} to {} to populate with weather station data...".format(
                    self.dates.normal_period_data_start_date,
                    self.dates.actual_data_end_date,
                )
            )
            index = pandas.date_range(
                start=self.dates.normal_period_data_start_date,
                end=self.dates.actual_data_end_date,
                freq="D",
            )
            self.finalDF = pandas.Series(index=index, dtype="object", name="value")
            ...
            while primary_station is None:
                ...
                if primary_station is not None:
                    ...
                    if float(self.site_lat) < 50:
                        ...
                        self.searchDistance += (
                            30  # In alaska it will probably go even higher.
                        )
                    if self.searchDistance <= maxSearchDistance:
                        ...
                        self.log.Wrap(
                            "Creating an empty dataset to populate with weather data..."
                        )
                        self.log.Wrap("")
                        index = pandas.date_range(
                            start=self.dates.normal_period_data_start_date,
                        ...
                        self.finalDF = pandas.Series(
                            index=index, dtype="object", name="value"
                        )
            ...
            maxNumberOfStations = (
                15  # Maximum number of stations to use to complete record
            )
            while (
                self.finalDF.isnull().sum().sum() > 0
                and num_stations_used < maxNumberOfStations
                and self.searchDistance <= maxSearchDistance
            ):
                # number_of_stations = len(self.stations)
                for station in self.stations:
                    if (
                        num_stations_used < maxNumberOfStations
                        and self.searchDistance <= maxSearchDistance
                        and self.gridded == False
                    ):
                        n += 1
                        if n == 1:
                            self.log.Wrap(
                                str(self.finalDF.isnull().sum().sum()) + " null values."
                            )
                        if self.searchDistance > 60:
                            need_primary = False
                        # Get baseline null value count
                        missing_before_normal = (
                            self.finalDF[
                                self.dates.normal_period_data_start_date : self.dates.normal_period_end_date
                            ]
                            .isnull()
                            .sum()
                            .sum()
                        )
                        missing_before_antecedent = (
                            self.finalDF[
                                self.dates.antecedent_period_start_date : self.dates.observation_date
                            ]
                            .isnull()
                            .sum()
                            .sum()
                        ...
                        self.log.Wrap(
                            "Attempting to replace null values with values from {}...".format(
                                station.name
                            )
                        )
                        # Fill
                        ...
                        if num_rows > 0:
                            ...
                            if self.save_folder is not None:
                                # Generate output
                                try:
                                    station_csv_name = "{}_{}.csv".format(
                                        station.name, self.dates.observation_date
                                    ).replace(
                                        "/", "_"
                                    )  # Slashes keep getting added to file names somehow, causing failures here
                                    station_csv_path = os.path.join(
                                        self.stationFolderPath, station_csv_name
                                    )
                                    if os.path.isfile(station_csv_path) is False:
                                        self.log.Wrap(
                                            "Saving station data to CSV in output folder..."
                                        )
                ...
                if self.finalDF.isnull().sum().sum() > 0:
                    self.log.Wrap("")
                    self.log.Wrap(
                        "No suitable station available to replace null values."
                    )
                    if float(self.site_lat) < 50:
                        self.searchDistance += 10  # Search distance increase interval
                    else:
                ...
                if self.finalDF.isnull().sum().sum() > 0:
                    ...
                    self.log.Wrap("")
                    self.log.Wrap(
                        "Attempting linear interpolation to fill null values..."
                    )
                    missing_before_normal = (
                        self.finalDF[
                            self.dates.normal_period_data_start_date : self.dates.normal_period_end_date
                    ...
                    self.log.Wrap(
                        str(self.finalDF.isnull().sum().sum())
                        + " null values remaining."
                    )
                    if self.finalDF.isnull().sum().sum() > 0:
                        self.log.Wrap(
                            "Unfortunately, data gaps still persist after interpolating. Therefore, the APT was unable to complete your analysis."
                        )
                    missing_after_normal = (
                        self.finalDF[
                            self.dates.normal_period_data_start_date : self.dates.normal_period_end_date
            ...
            if self.finalDF.isnull().sum().sum() < 1:
                ...
                if self.data_type != "PRCP":
                    self.log.Wrap(
                        'Since this is not for PRCP... filling null values with "0" to allow graph output...'
                    )
            ...
            if self.finalDF.isnull().sum().sum() > 0:
                self.log.Wrap(
                    "Null values remain after interpolation... the APT cannot complete this analysis"
                )
                self.log.Wrap("")
                raise
            ...
            if self.save_folder is not None:
                ...
                try:
                    ...
                    pass
                # Generate output
                merged_stations_output_path = os.path.join(
                    self.stationFolderPath,
                    "merged_stations_{0}.csv".format(self.dates.observation_date),
                )
                try:
                    if os.path.isfile(merged_stations_output_path) is False:
                        self.log.Wrap(
                            'Saving "merged_stations_{0}.csv" data to output folder...'.format(
                                self.dates.observation_date
                            )
                        )
            ...
            if self.save_folder is not None:
                # Generate output
                converted_stations_output_path = os.path.join(
                    self.stationFolderPath,
                    "merged_stations_converted_to_{0}_{1}.csv".format(
                        units, self.dates.observation_date
                    ),
                )
                if os.path.isfile(converted_stations_output_path) is False:
                    self.log.Wrap(
                        'Saving "merged_stations_converted_to_{0}_{1}.csv" data to output folder...'.format(
                            units, self.dates.observation_date
                        )
                    )
                    try:
            ...
            self.cdf_instance = netcdf_parse_all.get_point_history(
                float(self.site_lat),
            ...
            if os.path.isfile(outputName) is True:
                ...
                time.sleep(1)
            self.log.Wrap(
                "Saving complete gridded precipitation time series to CSV in output folder..."
            )
            pandas.concat(
                [
                    self.cdf_instance.entire_precip_ts,
                    self.cdf_instance.entire_station_count_ts,
                ],
                axis=1,
            ).to_csv(outputName, header=False)
            self.log.Wrap(
                "Entire TimeSeries rows = {}".format(
                    self.cdf_instance.entire_precip_ts.count()
                )
            )
            self.finalDF = self.cdf_instance.entire_precip_ts[
                self.dates.normal_period_data_start_date : self.dates.actual_data_end_date
            ]
            self.log.Wrap("FinalDF rows = {}".format(self.finalDF.count()))
            currentValues = self.cdf_instance.entire_precip_ts[
                self.dates.antecedent_period_start_date : self.dates.current_water_year_end_date
            ]
            ...
            if self.data_type == "PRCP":
                ...
                if self.finalDF is not None:
                    self.finalDF = self.finalDF * 0.03937008
                    self.log.Wrap("self.finalDF conversion complete.")
            station_count_data = numpy.float64(
                self.cdf_instance.entire_station_count_ts[
                    self.dates.normal_period_data_start_date : self.dates.actual_data_end_date
                ]
            )
        ...
        try:
            ...
            longRolling30day = pandas.rolling_sum(
                arg=self.finalDF, window=30, center=False
            )
        if self.data_type == "SNWD":
            longRolling30day = self.finalDF
        # Create version for calculating daily statistics
        statsRolling30day = longRolling30day[
            self.dates.normal_period_start_date : self.dates.normal_period_end_date
        ]
        # Create version for graphing current water year
        rolling30day = longRolling30day[
            self.dates.graph_start_date : self.dates.graph_end_date
        ]
        # get the max value
        ...
        normal_period_dates = pandas.date_range(
            self.dates.normal_period_start_date, self.dates.normal_period_end_date
        )
        # Convert to 365x30 table
        allDays = value_list_to_water_year_table(
            dates=normal_period_dates, values=statsRolling30day
        )

        # Get current-year Normals (And those same normals replicated over the previous and following years for graphing)
        self.log.Wrap(
            "Calculating Normal High and Normal Low values for each day of the year..."
        )
        # Create a list of dates in the selected range
        current_water_year_dates = pandas.date_range(
            self.dates.current_water_year_start_date,
            self.dates.current_water_year_end_date,
        )
        # Get all days Upper and Lower Normal values for the current water year
        current_normal_low_series, current_normal_high_series = calc_normal_values(
            dates=current_water_year_dates, values=allDays
        )
        # Create a list of dates for the prior water year
        prior_water_year_dates = pandas.date_range(
            self.dates.prior_water_year_start_date, self.dates.prior_water_year_end_date
        )
        # Get all days Upper and Lower Normal values for the current water year
        prior_normal_low_series, prior_normal_high_series = calc_normal_values(
            dates=prior_water_year_dates, values=allDays
        )
        # Create a list of dates for the following water year
        following_water_year_dates = pandas.date_range(
            self.dates.following_water_year_start_date,
            self.dates.following_water_year_end_date,
        )
        # Get all days Upper and Lower Normal values for the current water year
        following_normal_low_series, following_normal_high_series = calc_normal_values(
            dates=following_water_year_dates, values=allDays
        )
        ...
        try:
            first_point_y_rolling_total = abs(
                round(rolling30day.loc[first_point_x_date_string], 6)
            )
            ...
            pass
        try:
            second_point_y_rolling_total = abs(
                round(rolling30day.loc[second_point_x_date_string], 6)
            )
            ...
            pass
        try:
            third_point_y_rolling_total = abs(
                round(rolling30day.loc[third_point_x_date_string], 6)
            )
        ...
        self.log.Wrap(
            "Determining relationship between sample points and the normal range..."
        )
        if self.data_type == "PRCP":
            ...
            secondHigh = abs(
                round(normal_high_series.loc[second_point_x_date_string], 6)
            )
            ...
            if first_point_y_rolling_total > firstHigh:
                ...
                first_point_y_rolling_total <= firstHigh
                and first_point_y_rolling_total >= firstLow
            ):
            ...
            if second_point_y_rolling_total > secondHigh:
                ...
                second_point_y_rolling_total <= secondHigh
                and second_point_y_rolling_total >= secondLow
            ):
            ...
            if third_point_y_rolling_total > thirdHigh:
                ...
                third_point_y_rolling_total <= thirdHigh
                and third_point_y_rolling_total >= thirdLow
            ):
            ...
            try:
                ...
                if not wet_dry_season_result == "ERROR":
                    description_table_values.append(
                        [r"WebWIMP H$_2$O Balance", wet_dry_season_result]
                    )
        ...
        graph_start_datetime = datetime.strptime(
            self.dates.graph_start_date, "%Y-%m-%d"
        )
        ...
        ax1.set_ylabel("Rainfall ({})".format(units_long), fontsize=20)
        ...
        if self.data_type == "PRCP":
            ...
            if first_point_y_rolling_total:
                ...
                ax1.annotate(
                    first_point_label,
                    xy=(
                        pandas.Timestamp(first_point_x_date_string),
                        first_point_y_rolling_total,
                    ),
                    xycoords="data",
            ...
            if second_point_y_rolling_total:
                ...
                ax1.annotate(
                    second_point_label,
                    xy=(
                        pandas.Timestamp(second_point_x_date_string),
                        second_point_y_rolling_total,
                    ),
                    xycoords="data",
            ...
            if third_point_y_rolling_total:
                ...
                ax1.annotate(
                    third_point_label,
                    xy=(
                        pandas.Timestamp(third_point_x_date_string),
                        third_point_y_rolling_total,
                    ),
                    xycoords="data",
            ...
            description_table = ax3.table(
                cellText=description_table_values,
                colWidths=[0.4, 0.54],
        ...
        if self.data_type == "SNOW":
            ax1.set_title(
                "NOAA - National Climatic Data Center - Daily Global"
                " Historical Climatology Network - Snowfall Data",
                fontsize=20,
            )
        ...
        if self.data_type == "SNWD":
            ax1.set_title(
                "NOAA - National Climatic Data Center - Daily Global"
                " Historical Climatology Network - Snow Depth Data",
                fontsize=20,
            )
        ...
        if self.data_type == "SNOW" or self.data_type == "SNWD":
            ...
            # Plot Description Table
            description_table = ax3.table(
                cellText=description_table_values, colWidths=[0.4, 0.54], loc="center"
            )
            description_table.auto_set_font_size(False)
            description_table.set_fontsize(10)

            # Plot Stations Table
            stations_table = ax2.table(
                cellText=station_table_values,
            ...
            # Remove space between subplots
            plt.subplots_adjust(
                left=0.058, bottom=0.02, right=0.97, top=0.968, wspace=0.51, hspace=0.51
            )
