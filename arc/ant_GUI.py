...
class AntGUI(object):
    ...
    def __init__(self):
        ...
        self.gridded = False
        self.grid_selection = tkinter.Checkbutton(
            self.master,
        ...
        # ---LOCAL STREAMFLOW NORMALACY---#
        self.flow_norm_analysis = False
        self.flow_norm_analysis_selection = tkinter.Checkbutton(
            self.master,
            text="Calculate Local Streamflow Normal?",
            offvalue=0,
            onvalue=1,
            command=self.norm_flow_input,
        )
        self.flow_norm_analysis_selection.grid(
        ...
        separator = tkinter.ttk.Separator(
            self.master, orient="horizontal", style="Line.TSeparator"
        )
        ...
        self.label_watershed_scope.grid(
            row=self.row, column=2, sticky="ws", padx=3, pady=1
        )
        self.row += 1
        ...
        self.watershed_scope_menu = tkinter.ttk.OptionMenu(
            self.master,
        ...
        self.dates_frame.grid(
            row=self.row, column=0, sticky="nsew", padx=0, pady=0, columnspan=3
        )
        ...
        self.row += 1
        self.plus_button = tkinter.ttk.Button(
            self.dates_frame, command=self.add_date, image=self.PLUS_IMAGE
        )
        self.minus_button = tkinter.ttk.Button(
            self.dates_frame, image=self.minus_image, command=self.minus_function
        )
        ...
        # ---BOTTOM ROW BUTTONS---#
        self.BUTTON_CALCULATE = tkinter.ttk.Button(
            self.master, text="Calculate", command=self.calculate_and_graph
        )
        self.batch_style_string_var = tkinter.StringVar()
        self.batch_style_string_var.set("Switch to Date Range")
        self.BUTTON_BATCH = tkinter.ttk.Button(
            self.master,
            textvariable=self.batch_style_string_var,
            command=self.switch_batch_style,
        )
        self.BUTTON_QUIT = tkinter.ttk.Button(
            self.master, text="Quit", command=self.quit_command
        )
        self.BUTTON_CALCULATE.grid(
            row=99, column=0, padx=5, pady=5, columnspan=2, sticky="w"
        )
        ...
        self.LABEL_CUSTOM_WATERSHED_NAME = tkinter.ttk.Label(
            self.master, text="Custom Watershed Name:"
        )
        self.ENTRY_CUSTOM_WATERSHED_NAME = tkinter.ttk.Entry(self.master)
        self.LABEL_CUSTOM_WATERSHED_FILE = tkinter.ttk.Label(
            self.master, text="Custom Watershed Shapefile:"
        )
        self.ENTRY_CUSTOM_WATERSHED_FILE = tkinter.ttk.Entry(self.master)
        self.BUTTON_BROWSE_SHAPEFILE = tkinter.ttk.Button(
            self.master,
            text="Browse",
            command=self.ask_shapefile,
            image=self.folder_image,
        )

        # Reverse compatibility ITEMS (For the non-compiled version)
        self.BUTTON_BROWSE_DIR = tkinter.ttk.Button(
            self.master,
            text="Browse",
        ...
        self.RADIO_BUTTON_PARAMETER_RAIN = tkinter.ttk.Radiobutton(
            self.master,
            text="Rain",
            variable=self.RADIO_VARIABLE_PARAMETER,
            value="Rain",
        )
        self.RADIO_BUTTON_PARAMETER_SNOW = tkinter.ttk.Radiobutton(
            self.master,
            text="Snow",
            variable=self.RADIO_VARIABLE_PARAMETER,
            value="Snow",
        )
        self.RADIO_BUTTON_PARAMETER_SNOW_DEPTH = tkinter.ttk.Radiobutton(
            self.master,
            text="Snow Depth",
            variable=self.RADIO_VARIABLE_PARAMETER,
            value="Snow Depth",
        ...
        self.RADIO_BUTTON_Y_AXIS_VARIABLE = tkinter.ttk.Radiobutton(
            self.master,
            text="Variable",
            variable=self.RADIO_VARIABLE_Y_AXIS,
            value=False,
        )
        self.RADIO_BUTTON_Y_AXIS_CONSTANT = tkinter.ttk.Radiobutton(
            self.master,
            text="Constant",
            variable=self.RADIO_VARIABLE_Y_AXIS,
            value=True,
    ...
    def setup_unique_dates(self):
        # Add Upper Label
        self.upper_label_unique = tkinter.ttk.Label(
            self.dates_frame, text='Run a single date or click "+" to add more'
        )
        self.upper_label_unique.grid(
            row=0, column=0, columnspan=7, sticky="ew", padx=60
        )
        self.upper_label_separator = tkinter.ttk.Separator(
            self.dates_frame, orient="horizontal", style="Line.TSeparator"
        )
        ...
        self.top_row_separator = tkinter.ttk.Separator(
            self.dates_frame, orient="horizontal", style="Line.TSeparator"
        )
    ...
    def add_date(self):
        ...
        date_entry = DateEntry(
            self.dates_frame, font=("Helvetica", 10, "normal"), border=0
        )
        ...
        # ---SEPARATOR---#
        date_separator = tkinter.ttk.Separator(
            self.dates_frame, orient="horizontal", style="Line.TSeparator"
        )
    ...
    def setup_date_range(self):
        # Upper Label
        self.upper_label_range = tkinter.ttk.Label(
            self.dates_frame, text="Get daily results between a Start Date and End Date"
        )
        self.upper_label_range.grid(
            row=0, column=0, columnspan=3, sticky="nesw", padx=20
        )
        date_separator = tkinter.ttk.Separator(
            self.dates_frame, orient="horizontal", style="Line.TSeparator"
        )
        ...
        self.date_separators.append(date_separator)
        # ---START DATE---#
        date_label = tkinter.ttk.Label(
            self.dates_frame, text="Start Date (YYYY-MM-DD):"
        )
    ...
    def setup_csv_input(self):
        # Create/Grid
        self.upper_label_csv = tkinter.ttk.Label(
            self.dates_frame, text="Use a CSV file to run many dates at once"
        )
        ...
        self.label_csv_file_path = tkinter.ttk.Label(
            self.dates_frame, text="CSV File Path:"
        )
        self.entry_csv_file_path = tkinter.ttk.Entry(self.dates_frame)
        self.button_browse_csv_file_path = tkinter.ttk.Button(
            self.dates_frame,
            text="Browse",
            command=self.ask_csv_file,
            image=self.folder_image,
        )
    ...
    def watershed_selection(self, event):
        ...
        if watershed_scale == "Custom Polygon":
            ...
            self.LABEL_CUSTOM_WATERSHED_NAME.grid(
                row=94, sticky="sw", padx=5, pady=3, columnspan=3
            )
            self.ENTRY_CUSTOM_WATERSHED_NAME.grid(
                row=95, padx=5, sticky="ew", columnspan=3
            )
            self.LABEL_CUSTOM_WATERSHED_FILE.grid(
                row=96, sticky="sw", padx=5, pady=3, columnspan=3
            )
            self.ENTRY_CUSTOM_WATERSHED_FILE.grid(
                row=97, padx=5, sticky="ew", columnspan=3
            )
            self.BUTTON_BROWSE_SHAPEFILE.grid(
                row=97, column=2, padx=4, pady=1, sticky="e"
            )
        else:
    ...
    def norm_flow_input(self):
        self.flow_norm_analysis = not self.flow_norm_analysis
    ...
    def test_noaa_server(self):
        ...
        if self.ncdc_working is False:
            try:
                ...
                self.L.Wrap(
                    "Server Base URL = https://www1.ncdc.noaa.gov/pub/data/ghcn/daily"
                )
                ...
                for i in range(5):
                    if i < 1:
                        ...
                        self.L.Wrap(
                            "  Attempt {} of 5 - Downloading: {}".format(
                                (i + 1), test_url
                            )
                        )
        ...
        if self.ncdc_working is False:
            ...
            for i in range(5):
                ...
                if i < 1:
                    ...
                    try:
                        self.L.Wrap(
                            "   -Creating FTP Instance for {}...".format(ftp_address)
                        )
                        ftp = ftplib.FTP(ftp_address)
                        self.L.Wrap("   -Logging in to FTP Instance...")
                        ftp.login()
                        self.L.Wrap(
                            "   -Navigating to selected folder ({})...".format(
                                ftp_folder
                            )
                        )
                        ftp.cwd(ftp_folder)
                        ...
                        if (
                            dly_size < 300
                        ):  # 241 is what the 403 Forbidden error has been producing during Gov't Shutdown
    ...
    def calculate_and_graph(self):
        ...
        if self.ncdc_working is True and self.gridded is False:
            try:
                current_style = self.batch_style_string_var.get()
                if (
                    current_style == "Switch to Date Range"
                ):  # Means it is currently on Unique
                    self.get_inputs_unique()
                elif (
                    current_style == "Switch to CSV Input"
                ):  # Means it is currently on Range
                    self.get_inputs_range()
                elif (
                    current_style == "Switch to Unique Dates"
                ):  # Means it is currently on CSV
                    self.get_inputs_csv()
            except Exception:
    ...
    def test_parameters(
        ...
        if custom_watershed_file != "":
            ...
            if watershed_extension.lower() == ".shp":
                ...
                if not prj_exists:
                    self.L.Wrap(
                        "SUPPLIED CUSTOM WATERSHED FILE LACKS REQUIRED PROJECTION (.prj) FILE!"
                    )
                    parameters_valid = False
            if custom_watershed_name == "":
                custom_watershed_name = os.path.splitext(
                    os.path.split(custom_watershed_file)[1]
                )[0]
        else:
        ...
        if parameters_valid:
            ...
            if observation_datetime > two_days_prior_datetime:
                ...
                self.L.Wrap(
                    "  Observation date updated to: {}".format(observation_date)
                )
    ...
    def calculate_or_add_batch(self, batch, params):
        ...
        if test is False:
            ...
            if batch is True:
                self.L.Wrap(
                    "The selected inputs have already been added to the batch list."
                )
            self.L.Wrap("")
        if batch is False:
            # -WATERSHED START
            if radio == "Rain":
                ...
                if watershed_scale != "Single Point":
                    ...
                    if len(input_list_list) > 1:
                        ...
                        self.L.Wrap(
                            'Manual batch processing lists are not supported for Watershed Scales other than "Single Point"'
                        )
                        self.L.Wrap(
                            "  Clearing Batch Process Queue to prepare for Watershed Random Sampling Points..."
                        )
                        input_list_list = []
                    ...
                    if watershed_scale != "Custom Polygon":
                        ...
                        huc, sampling_points, huc_square_miles = (
                            huc_query.id_and_sample(
                                lat=latitude,
                                lon=longitude,
                        ...
                        sampling_points, huc_square_miles = (
                            custom_watershed_query.shapefile_sample(
                                lat=latitude,
                                lon=longitude,
                    ...
                    self.L.Wrap(
                        "Adding Random Sampling Points for the watershed to the Batch Process Queue..."
                    )
                    if watershed_scale != "Custom Polygon":
                        self.L.Wrap(
                            "{} ({}) - Watershed Sampling Points:".format(
                                watershed_scale, huc
                            )
                        )
                    else:
                        self.L.Wrap(
                            "{} ({}) - Watershed Sampling Points:".format(
                                watershed_scale, custom_watershed_name
                            )
                        )
                    for sampling_point in sampling_points:
                        ...
                        self.L.Wrap(
                            " Sampling Point {} - {}".format(
                                str(len(input_list_list)), str(input_list)
                            )
                        )
                    self.L.print_separator_line()
            # -WATERSHED END
            ...
            if not len(input_list_list) > 1:
                ...
                if self.flow_norm_analysis:
                    ...
                    local_norm(
                        float(run_list[1]),
                        float(run_list[2]),
                        f"{run_list[3]}-{run_list[4]}-{run_list[5]}",
                        str(output_folder),
                    )
                ...
                if save_folder is None:
                    ...
                    self.L.Wrap(
                        "Setting Output Folder to default location: {}...".format(
                            save_folder
                        )
                    )
                ...
                if radio == "Rain":
                    if watershed_scale == "Single Point":
                        ...
                        output_folder = os.path.join(version_folder, coord_string)
                        # Define PDF Outputs
                        final_path_variable = os.path.join(
                            output_folder,
                            "({}, {}) Batch Result.pdf".format(latitude, longitude),
                        )
                        final_path_fixed = os.path.join(
                            output_folder,
                            "({}, {}) Batch Result - Fixed.pdf".format(
                                latitude, longitude
                            ),
                        )
                        # Define CSV Output
                        csv_path = os.path.join(
                            output_folder,
                            "({}, {}) Batch Result.csv".format(latitude, longitude),
                        )
                    elif watershed_scale == "Custom Polygon":
                        watershed_analysis = True
                        version_folder = os.path.join(save_folder, VERSION_FOR_PATHS)
                        general_watershed_folder = os.path.join(
                            version_folder, "~Watershed"
                        )
                        watershed_scale_folder = os.path.join(
                            general_watershed_folder, watershed_scale
                        )
                        output_folder = os.path.join(
                            watershed_scale_folder, custom_watershed_name
                        )
                        watershed_analysis = True
                        # Define PDF Outputs
                        final_path_variable = os.path.join(
                            output_folder,
                            "{} - {} - Batch Result.pdf".format(
                                observation_date, custom_watershed_name
                            ),
                        )
                        watershed_summary_path = os.path.join(
                            output_folder,
                            "{} - {} - Summary Page.pdf".format(
                                observation_date, custom_watershed_name
                            ),
                        )
                        final_path_fixed = os.path.join(
                            output_folder,
                            "{} - {} - Batch Result - Fixed Scale.pdf".format(
                                observation_date, custom_watershed_name
                            ),
                        )
                        # Define CSV Output
                        ...
                        final_path_variable = os.path.join(
                            output_folder,
                            "{} - {} - Batch Result.pdf".format(observation_date, huc),
                        )
                        watershed_summary_path = os.path.join(
                            output_folder,
                            "{} - {} - Summary Page.pdf".format(observation_date, huc),
                        )
                        final_path_fixed = os.path.join(
                            output_folder,
                            "{} - {} - Batch Result - Fixed Scale.pdf".format(
                                observation_date, huc
                            ),
                        )
                        # Define CSV Output
                        csv_path = os.path.join(
                    ...
                    final_path_fixed = os.path.join(
                        output_folder,
                        "({}, {}) Batch Result - Fixed.pdf".format(latitude, longitude),
                    )
                    # Define CSV Output
                    csv_path = os.path.join(
                        output_folder,
                ...
                # Create csv_writer
                csv_writer = JLog.PrintLog(
                    Delete=True, Log=csv_path, Indent=0, Width=400, LogOnly=True
                )
                # Write first line of CSV
                if watershed_scale == "Single Point":
                    csv_writer.Wrap(
                        "Latitude,Longitude,Date,PDSI Value,PDSI Class,Season,Antecedent Precip Score,Antecedent Precip Condition"
                    )
                else:
                    csv_writer.Wrap(
                        "Latitude,Longitude,Date,PDSI Value,PDSI Class,Season,Antecedent Precip Score,Antecedent Precip Condition"
                ...
                for current_input_list in current_input_list_list:
                    ...
                    if watershed_scale == "Single Point":
                        ...
                        self.L.print_title(
                            "Single Point Batch Analysis - Date {} of {}".format(
                                run_count, total_pdfs
                            )
                        )
                    else:
                        self.L.print_title(
                            "{} WATERSHED ANALYSIS - SAMPLING POINT {} of {}".format(
                                watershed_scale, run_count, total_pdfs
                            )
                        )
                    self.L.Wrap("")
                    ...
                    (
                        ...
                        all_sampling_coordinates=sampling_points,
                        gridded=self.gridded,
                    )
                        ...
                        if total_pdfs > 1:
                            ...
                            if len(pdf_list) > 365:
                                if (total_pdfs - pdf_count) > 25:
                                    ...
                                    self.L.Wrap("")
                                    self.L.Wrap(
                                        "Merging PDFs to temp file to avoid crash at the end from merging too many files at once..."
                                    )
                                    self.L.Wrap("")
                                    # Determine available temp file name
                                    final_path_variable_part = (
                                        "{} - Part {}.pdf".format(
                                            final_path_variable[:-4], part_count
                                        )
                                    )
                                    # Merge current PDFs
                ...
                if watershed_scale != "Single Point":
                    ...
                    generated = watershed_summary.create_summary(
                        site_lat=latitude,
                        site_long=longitude,
                ...
                if pdf_list:  # Testing list for content
                    ...
                    if fixed_y_max is True:
                        ...
                        for current_input_list in current_input_list_list:
                            self.L.Wrap("")
                            self.L.Wrap(
                                "Re-running with fixed yMax value: "
                                + str(current_input_list)
                            )
                            self.L.Wrap("")
                            (
                                result_pdf,
                            ...
                            if len(pdf_list) > 365:
                                if (total_pdfs - pdf_count) > 25:
                                    ...
                                    self.L.Wrap("")
                                    # Determine available temp file name
                                    final_path_fixed_part = "{} - Part {}.pdf".format(
                                        final_path_fixed[:-4], part_count
                                    )
                                    # Merge current PDFs
                                    merger = PyPDF2.PdfFileMerger()
                                    for doc in pdf_list:
                ...
                if radio == "Rain":
                    ...
                    self.input_list_list_snwd = []
            self.L.Wrap("")
            self.L.Time(StartTime=start_time, Task="All tasks")
            self.L.Wrap("")
...
class DateEntry(tkinter.Frame):
    ...
    def __init__(self, master, frame_look={}, **look):
        ...
        self.three_days_prior_string = self.three_days_prior_datetime.strftime(
            "%Y-%m-%d"
        )
        ...
        self.label_yr_mo = tkinter.ttk.Separator(
            self, orient="horizontal", style="Line.TSeparator"
        )
        self.entry_month = tkinter.ttk.Entry(self, width=2)
        self.label_mo_dd = tkinter.ttk.Separator(
            self, orient="horizontal", style="Line.TSeparator"
        )
            ...
            try:
                ...
                if self.gridded is False:
                    if entry_datetime > self.two_days_prior_datetime:
                        print("")
                        print(
                            "INPUT ERROR - GHCN Data is two days old - Please change to: ({})".format(
                                self.two_days_prior_string
                            )
                        )
                ...
                if self.gridded is True:
                    if entry_datetime > self.three_days_prior_datetime:
                        print("")
                        print(
                            "INPUT ERROR - GHCN Data is three days old - Please change to: ({})".format(
                                self.three_days_prior_string
                            )
                        )
                        self.date_problem = True
            except Exception as error:
        ...
        if len(entry_text) > 3:
            ...
            if not self.gridded:
                if int_year < 1910:
                    ...
                    print(" ")
                    print(
                        "Year cannot be greater than {}!".format(
                            self.two_days_prior_year
                        )
                    )
            else:
                if int_year < 1983:
