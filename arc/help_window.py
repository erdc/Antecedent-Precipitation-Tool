...
ABOUT_HELP_TEXT = """
This tool was created to facilitate the comparison of the precipitation at a given location and date to the normal 
precipitation range at that location over the preceding 30 years. Updates to this tool may be made over time, and will 
be indicated by a change in the version number and date in the on-screen display and in the documentation generated by 
the tool.

For questions regarding the day-to-day use of the APT or CWA Policy or Implementation, contact the USEPA at: 
CWAwotus@EPA.GOV

To report errors with this program, click the "Report Issue" button or email:  
APT-Report-Issue@usace.army.mil
"""

ABOUT_HELP_TEXT += "STUFF"
...
class HelpWindow(object):
    ...
    def __init__(self):
        ...
        self.first_text = tkinter.Text(self.first_frame, height=10, width=100)
        self.scrollbar1 = tkinter.ttk.Scrollbar(self.first_frame) # making a scrolbar with entry test text field
        self.first_text.config(yscrollcommand=self.scrollbar1.set) # setting scrolbar to y-axis
        self.scrollbar1.config(command=self.first_text.yview) # setting the scrolbar to entry test textbox
        ...
        #'''# ---SECOND FRAME---
        self.second_frame = ttk.Frame(self.primary_frame)
        self.second_frame.grid(row=self.row, column=0, sticky="nsew", padx=25, pady=0)
        self.second = tkinter.Canvas(self.second_frame, height=10, width=119)
        self.canvas.pack(side="left", fill="both", expand=True)
        self.scrollbar2 = tkinter.ttk.Scrollbar(self.second_frame) # making a scrolbar with entry test text field
        self.second.config(yscrollcommand=self.scrollbar2.set) # setting scrolbar to y-axis
        self.scrollbar2.config(command=self.second.yview) # setting the scrolbar to entry test textbox
        # self.second.insert('end', LINKS_TEXT) # Inserting the About/Help Text
        self.second.config(state='disabled')
        self.second.grid(column=0, row=self.row, sticky='nsew') 
        self.scrollbar2.grid(column=1, row=self.row, sticky='nsw')

        docs_folder = os.path.join(root_folder, 'help')
        self.add_reference(frame=self.second,
        ...
        self.add_reference(frame=self.second,
                           ...
                           youtube_url=None)
        self.add_separator(self.second_frame) #'''


        '''# ---THIRD TEXT FRAME---
        self.third_frame = ttk.Frame(self.primary_frame)
        self.third_frame.grid(row=self.row, column=0, sticky="nsew", padx=25, pady=0)
        self.third_text = tkinter.Text(self.third_frame, height=10, width=119)
        self.scrollbar2 = tkinter.ttk.Scrollbar(self.third_frame) # making a scrolbar with entry test text field
        self.third_text.config(yscrollcommand=self.scrollbar2.set) # setting scrolbar to y-axis
        self.scrollbar2.config(command=self.third_text.yview) # setting the scrolbar to entry test textbox
        self.third_text.insert('end', ABOUT_HELP_TEXT) # Inserting the About/Help Text
        self.third_text.config(state='disabled')
        self.third_text.grid(column=0, row=self.row, sticky='nsew') 
        self.scrollbar2.grid(column=1, row=self.row, sticky='nsw') 
        self.add_separator(self.third_frame) #'''
    ...
    def add_reference(self, frame, title, pdf_local_path=None, pdf_url=None, youtube_url=None):
        ...
        if youtube_url is not None:
            ...
            youtube_button = ttk.Button(frame,
                                        text=youtube_text,
