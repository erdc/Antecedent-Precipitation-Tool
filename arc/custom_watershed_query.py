...
def shapefile_sample(lat, lon, shapefile):
    ...
    log.Wrap(
        " -Filtering HUC8 features by spatial overlap with selected coordinates..."
    )
    ...
    if horizontal_units.lower() in supported_units:
        ...
        elif horizontal_units.lower() in [
            "foot",
            "feet",
            "us feet",
            "us foot",
    ...
    if horizontal_units.lower() in ["meter", "meters", "metre"]:
        sampling_point_spacing = (
            sampling_point_spacing_miles * 1609.34
        )  # 1609.34 Meters = 1 Mi
    elif horizontal_units.lower() in [
        "foot",
        "feet",
    ...
    log.Wrap(
        " -Latitudes and Longitudes will be randomly generated watershed polygon extremes:"
    )
    log.Wrap(
        "   -Custom Watershed Coordinate Extremes (Converted to Meters for testing):"
    )
    ...
    log.Wrap(
        " -An OGR point geometry will be created to test each random Latitude and Longitude."
    )
    log.Wrap("   -The point must fall Within the Custom Watershed provided")
    log.Wrap(
        "   -The point must also be at least {} mile(s) from any previously selected sampling points.".format(
            sampling_point_spacing_miles
        )
    )
    log.Wrap(
        " -If both criteria are met, the point will be added to the list of sampling points."
    )
    log.Wrap(
        " -When 3000 consecutive random test points fail these tests, the sampling procedure will be complete."
    )
    ...
    while num_points > 0:
        ...
        if points_tested_since_last_success > 3000:
            if num_points < 997:
                log.Wrap(
                    "Sampling complete (3000 consecutive points tested since the last suitable one was found)."
                )
                break
            else:
                log.Wrap(
                    "  {} points selected of {} test candidates generated.".format(
                        points_selected, points_tested
                    )
                )
                points_tested_since_last_success = 0
                log.Wrap(
                    "3000 points tested since the last suitable one was found.  Lowering minimum spacing by 0.5 mile."
                )
                sampling_point_spacing_miles = round(
                    (sampling_point_spacing_miles - 0.5), 2
                )
                if horizontal_units.lower() in ["meter", "meters", "metre"]:
                    sampling_point_spacing = (
                        sampling_point_spacing_miles * 1609.34
                    ...
                    sampling_point_spacing = (
                        sampling_point_spacing_miles * 5280
                    )  # 5280 Feet = 1 Mi
                log.Wrap(
                    " -Now each point must be at least {} miles(s) from all other sampling points".format(
                        sampling_point_spacing_miles
                    )
                )
        log.print_status_message(
            "  {} points selected of {} test candidates generated. Testing ({}, {})".format(
                points_selected, points_tested, test_y_round, test_x_round
            )
        )
        ...
        if inside:
            ...
            if properly_spaced:
                ...
                if not original_horizontal_units.lower() in supported_units:
                    [wgs_lon, wgs_lat, z] = transform_albers_to_wgs.TransformPoint(
                        test_x, test_y
                    )
                else:
    ...
    log.Wrap(
        "{} sampling points selected from {} randomly generated candidates".format(
            points_selected, points_tested
        )
    )
...
if __name__ == "__main__":
    ...
    sampling_points, huc_square_miles = shapefile_sample(
        lat=38.3315574, lon=-121.3622894, shapefile=SHAPEFILE
    )
