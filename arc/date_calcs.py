...
class DateCalc(object):
    ...
    def calculate(self, gridded):
        ...
        if int(self.observation_month) > 9:
            ...
            self.following_water_year_start_date = (
                str(self.observation_year + 1) + "-10-01"
            )
            self.following_water_year_end_date = (
                str(self.observation_year + 2) + "-09-30"
            )
            self.normal_period_start_date = str(self.observation_year - 31) + "-10-01"
            self.normal_period_data_start_date = (
                str(self.observation_year - 31) + "-09-01"
            )
            self.normal_period_end_date = str(self.observation_year) + "-09-30"
        else:
            self.current_water_year_start_date = (
                str(self.observation_year - 1) + "-10-01"
            )
            ...
            self.following_water_year_end_date = (
                str(self.observation_year + 1) + "-09-30"
            )
            self.normal_period_start_date = str(self.observation_year - 32) + "-10-01"
            self.normal_period_data_start_date = (
                str(self.observation_year - 32) + "-09-01"
            )
            self.normal_period_end_date = str(self.observation_year - 1) + "-09-30"
        # Calculate antecedent period start (90 days prior to obs_date)
        antecedent_period_start_datetime = self.observation_datetime - timedelta(
            days=89
        )
        self.antecedent_period_start_date = antecedent_period_start_datetime.strftime(
            "%Y-%m-%d"
        )
        ...
        while last_day_of_month < 32:
            last_day_of_month += 1
            test_date = "{}-{}-{}".format(
                graph_end_year, graph_end_month, last_day_of_month
            )
            try:
