...
station_table_values = [
    ...
    ["SEARCHLIGHT", "35.4661, -114.9217", 3540.026, 40.256, 724.604, 47.285, 11190, 90],
]  # 1
station_table_values.append(
    ["MITCHELL CAVERNS", "34.9436, -115.5469", 4350.066, 10.307, 85.436, 5.519, 80, 0]
)  # 2
station_table_values.append(
    ["MTN PASS", "35.4703, -115.5436", 4729.987, 30.708, 465.357, 28.109, 1, 0]
)  # 3
station_table_values.append(
    ["MTN PASS 1SE", "35.4592, -115.5311", 4776.903, 29.794, 512.273, 28.67, 56, 0]
)  # 4
station_table_values.append(
    ["BAKER", "35.2761, -116.0628", 961.942, 40.437, 3302.688, 151.747, 4, 0]
)  # 5
station_table_values.append(
    ["AMBOY", "34.5592, -115.7442", 647.966, 38.318, 3616.644, 155.826, 20, 0]
)  # 6
station_table_values.append(
    ["NEEDLES AP", "34.7675, -114.6189", 890.092, 48.53, 3374.538, 185.605, 2, 0]
)  # 7
...
if data_type == "PRCP":
    ...
    logoFile = os.path.join(images_folder, "RD_2_1.png")
...
if data_type == "PRCP":
    ax1.set_title(
        "Antecedent Precipitation and 30-Year Normal Range from NOAA's Daily Global Historical Climatology Network",
        fontsize=20,
    )
    ...
    if first_point_y_rolling_total is not None:
        ...
        ax1.annotate(
            ...
            arrowprops=dict(
                arrowstyle="simple", fc="0.4", ec="none", connectionstyle="arc3,rad=0.5"
            ),
        )
...
row_labels = ["1st Prior 30", "2nd Prior 30", "3rd Prior 30", "Result"]
rain_table_vals = [
    [
        "30 Days Ending",
        r"30$^{th}$ %ile" + "  ({})".format(units),
...
plt.subplots_adjust(
    wspace=0.00, hspace=hValue, left=0.045, bottom=bValue, top=0.968, right=0.99
)
