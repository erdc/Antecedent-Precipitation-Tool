...
def ensure_WIMP():
    ...
    if os.path.exists(wimp_path):
        return

    # find local zip, raises if not found
    local_file_path = find_file_or_dir(ROOT_FOLDER, "WebWimpcache.zip", num_searches=1)

    # create folder
    extract_local_archive(zip_path=local_file_path, extract_path=wimp_folder)
